...
In \Cref{sec:one_sided}, I present the main results for the case of one indispensable player and a continuum of identical fringe players.
\Cref{sec:many_sided} extends these results to the case of multiple types of fringe players.
...
\begin{proposition}
    ...
    Let $f$ be continuous on [0, 1]. Furthermore, let us denote the random variable $\frac{|\mathcal{P}_P|}{n}$ as $X_n$. Assume that $X_n \xrightarrow[]{d} X$ with cdf $G(t)$ and (if exists) pdf $g(t)$.
    Then
    \begin{align*}
...
\begin{proposition}
    ...
    Furthermore, let us denote the random variable $\frac{n_A(\mathcal{P}_{P_j})}{n}$ as $X_n^j$.
...
The number of actual entrants depends on the utility they would derive from participating in the market.\footnote{
    ...
    For simplicity, I refrain from explicitly modeling this cost in the main text, but \Cref{sec:explicit_entry_costs} provides a discussion of how one can incorporate it into the model, its impact on bargaining outcomes.
}
...
As it turns out, these shares only depend on the bargaining weights ($\lambda_P, \lambda_1, \lambda_2$) and the network effects ($\gamma_1, \gamma_2$), but not on the actual number of entrants.\footnote{
    ...
    Just like in \Cref{sec:power_function_example}, each constituent of the polynomial are divided in a constant proportion.
}
...
\begin{proof}[Proof of \Cref{prop:multiple_platforms_total}]
    ...
    \begin{align*}
        \varphi^{n, m}_P &= \sum_{j=1}^m \sum_R \Pr(R) [ v(\mathcal{P}^R_{P_j} \cup \{i\}) - v(\mathcal{P}^R_{P_j}) ] \\
        &= \sum_{j=1}^m \sum_{k=0}^n \Pr(n_A(\mathcal{P}^R_{P_j}) = k \land n_P(\mathcal{P}^R_{P_j}) = 0) f(k/n),
    \end{align*}
    where $n_A(S)$ and $n_P(S)$ denote the number of fringe players and big players in coalition $S$, respectively.
    This can be further rewritten as
    \begin{align*}
        \varphi^{n, m}_P &= \sum_{j=1}^m \sum_{k=0}^n \Pr(n_A(\mathcal{P}^R_{P_j}) = k \mid n_P(\mathcal{P}^R_{P_j}) = 0) \Pr(n_P(\mathcal{P}^R_{P_j}) = 0) f(k/n) \\
        &= \sum_{k=0}^n \Pr(n_A(\mathcal{P}^R_{P_j}) = k \mid n_P(\mathcal{P}^R_{P_j}) = 0) f(k/n),
    \end{align*}
    where the last equality follows from the fact that the big players are identical in terms of permutation probabilities.

    Remember that the share of fringe players before player $j$ is described by the random variable $X_n^j = n_A(\mathcal{P}^R_{P_j}) / n$.
    Observe that
    \begin{align*}
        \Pr(X_n^j = k/n \mid n_P(\mathcal{P}^R_{P_j}) = 0) &= \Pr(X_n^j = k/n \mid X_n^j = \min\{X_n^1, \dots, X_n^m\} \land n_P(\mathcal{P}^R_{P_j}) = 0) \\
        &= \Pr(\min\{X_n^1, \dots, X_n^m\} = k/n \mid n_P(\mathcal{P}^R_{P_j}) = 0) \\
    ...
    \begin{align*}
        \varphi^{n, m}_P &= \sum_{k=0}^n \Pr(n_A(\mathcal{P}^R_{P_j}) \mid n_P(\mathcal{P}^R_{P_j}) = 0) f(k/n) \\
