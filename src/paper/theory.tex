...
Assume that the total utility of a player on side $i$ also includes an idiosyncratic component.
Let us order the players on side $i$ by this idiosyncratic utility in a decreasing manner.
That is, if we denote the idiosyncratic utility of player $n$ on side $i$ as $\epsilon_i(n)$, then $\epsilon_i$ is strictly decreasing in $n$.
Also assume that $\lim_{n \to \infty} \epsilon_i(n) = -\infty$.

Entry is given by the usual assumption that a player enters if and only if the total utility of entering is non-negative.

Due to the properties of $\epsilon_i$, this implies that there is a well-defined, finite number of entrants on each side.
...
\begin{align*}
    w_i^*(n_i, n_j) = \alpha_i n_i n_j ^ {\gamma_i} + \gamma_j \alpha_j n_j n_i^{\gamma_j} - n_i F_i + \int_0^{n_i} \epsilon_i(s) \ds.
\end{align*}
...
\begin{align*}
    ...
    &= \alpha_i n_j ^ {\gamma_i} + \alpha_j \gamma_j n_j n_i^{\gamma_j - 1} - F_i - n_i \epsilon'_i(n_i).
\end{align*}
Welfare of side $i$ immediately follows in this case, too:
\begin{align*}
    U_i^u(n_1, n_2) = \alpha_i n_i n_j ^ {\gamma_i} + \gamma_j \alpha_j n_j n_i^{\gamma_j} - n_i F_i - n_i^2 \epsilon'_i(n_i) + \int_0^{n_i} \epsilon_i(s) \ds.
\end{align*}
...
\begin{proposition}
    ...
    \begin{alignat*}{4}
        ...
        U_1^b(n_1, n_2) &= \underbracket{\frac{\lambda_1}{\lambda_P + \lambda_1 + \lambda_2\gamma_1}}_{\coloneqq S_1^{b_1}} \alpha_1 n_1 n_2^{\gamma_1} &&+ \underbracket{\frac{\lambda_1 \gamma_2}{\lambda_P + \lambda_2 + \lambda_1\gamma_2}}_{\coloneqq S_1^{b_2}}  \alpha_2 n_2 n_1^{\gamma_2} &&- \underbracket{\frac{\lambda_1}{\lambda_P + \lambda_1}}_{\coloneqq S_1^{c_1}} n_1 F_1 && \\
        U_2^b(n_1, n_2) &= \underbracket{\frac{\lambda_2\gamma_1}{\lambda_P + \lambda_1 + \lambda_2\gamma_1}}_{\coloneqq S_2^{b_1}} \alpha_1 n_1 n_2^{\gamma_1} &&+ \underbracket{\frac{\lambda_2}{\lambda_P + \lambda_2 + \lambda_1\gamma_2}}_{\coloneqq S_2^{b_1}} \alpha_2 n_2 n_1^{\gamma_2} && &&- \underbracket{\frac{\lambda_2}{\lambda_P + \lambda_2}}_{\coloneqq S_2^{c_2}} n_2 F_2.
    \end{alignat*}
\end{proposition}
...
Let us look at the per-player utilities to 
