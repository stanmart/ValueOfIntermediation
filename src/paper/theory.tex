...
\begin{align*}
    ...
    \end{cases}
\end{align*}
The function $f$ (henecforth also referred to as the production function) then characterizes this game, and determines the distribution of the value between the big player and the fringe.
...
Therefore, for any coalition of fringe players, the marginal contribution of player $P$ is the total value that the coalition can generate $P$ included.
Player $P$'s then is its average marginal contribution, which boils down to the expected value that a coalition can generate conditional on player $P$ being in it.
...
This marginal contribution is the derivative of the production function with respect to the mass of fringe players (i.e., the effect of adding an infinitesimally small player to the coalition), multiplied by the probability of the central player appearing before the fringe player (otherwise the marginal contribution is zero, as the central player is indispensable).
The second interpretation is particularly useful when thinking about the value of the fringe in the setting with heterogeneous fringe firms.
...
In certain settings, this can also be interpreted as the degree of complementarity or substitutability between the fringe players.
For example, imagine that the fringe consists of a set of firms producing one product each, and the central player is necessary for this product to be sold or created.\footnote{
    For example, the central player could be a platform that connects the fringe firms with consumers, or a provider of a key input.
}
When the fringe firms are substitutes, then additional ones add less value to the total pie.
Therefore, relatively few of them are needed to create most of the value, and the central player can appropriate a larger share.
...
The weights can be thought of as the measure of some innate bargaining power, not reflected in the production function.\footnote{
...
In both cases, the player's shares are still random order values\footnote{
    The case of multiple big players is equivalent to the case of a single big player with the same production function, but a different probability distribution over the permutations.
...
Namely, it also depends on the shape of function.
...
I approach this problem by breaking it down into two parts: bargaining over the idisyncratic and non-idiosyncratic parts of the value generated.
...
Furthermore, the non-idiosyncratic part is the same as in the main text, and thus the weighted values arising from it are also identical.
