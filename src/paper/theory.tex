...
\begin{proposition}
    ...
    Let us denote the number of fringe players in any coalition $S$ as $n_F(S)$.
    Furthermore, let us denote the random variable $\frac{n_F(\mathcal{P}_{P_j})}{n}$ as $X_n$.
    Assume that $X_n \xrightarrow[]{d} X$ with cdf $G(t)$ and (if exists) pdf $g(t)$.
...
\begin{corollary}
    ...
    Let $\varphi_{P}^{\infty, m}$ denote the aggregated values of players of type $P$. $\varphi_{P}^{\infty, m}$ is decreasing in $m$ and $\varphi_{A}^{\infty, m}$ is increasing in $m$ unless $f$ is constant.
\end{corollary}
% TODO: adjust proof
