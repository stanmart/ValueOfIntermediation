...
% \pgfplotstableread[col sep=comma]{out/figures/equilibrium_profit_onesided_scale-1_lambda-1.csv}\equilibriumProfitOneSided
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[color=myblue,mark=none,thick] table[x=N_P,y=F_F_implied]{\equilibriumProfitOneSidedLowLambda};
                \addplot[color=black,mark=none,thick,dashed] table[x=N_P,y=F_F_opt]{\equilibriumProfitOneSidedLowLambda};
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Optimal and actual (implied) entry fees}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{tikzpicture}
            \begin{axis}[
                    ...
                    ylabel={},
                    xmin=0, xmax=1.5, % adjust these values as needed
                    ymin=0.25, ymax=0.35, % adjust these values as needed
                    axis lines=left,
                    xtick={0, 1.5},
                    legend pos=north west,
                    width=\textwidth,
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Platform profits net of investment costs}
    \end{subfigure}
    \caption{Equilibrium outcomes in the case when the platform has lower innate bargaining power ($N_C = 1, V_P = 1, V_F = 1, I_F = 0.2$). As in every other case, the platform's profits are increasing in $N_P$, while the fringe's profits are decreasing. Furthermore, consumer surplus is also decreasing as a result in a decrease in total product variety. The shaded region represents hybrid operation, while in the unshaded region, the platform operates in pure retail mode.}
