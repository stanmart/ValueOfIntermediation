...
Platforms have a number of unique features, which make them different from more traditional market structures.
...
However, in the more recent years, another important and potentially concerning behavior has been getting more attention: the hybrid operation of platforms.
...
Perhaps the most prominent example is Amazon, which on the one hand selling its own products, and at the same time is hosting  large number of third-party sellers.
Examples abound in other industries, as well.
For example, all of the largest digital distribution platforms for computer and smartphone applications (such as Google Play, Apple App Store or Microsoft Store) sell applications products by the platform owner itself besides their third party offerings.
In the video game industry, platform owners owning their own game studios is the standard.
...
For example BMW is preparing to offer cars directly to consumers in the US, competing with its own dealership network.

As deciding the rules of the game and competing in it at the same time gives rise to some obvious concerns, policymakers have been paying a closer look at these platforms in the recent years.
...
Furthermore, one of the most prominent ongoing antitrust cases (in 2022--2023), Microsoft attempting to acquire the video game publisher Activision/Blizzard for a sum of 69 billion USD, is also a case of a platform becoming increasingly more hybrid.
This merger would not only increase market concentration on the software side of the video game industry, but might also have further implications for bargaining power between Microsoft and the other game developers.
The deal currently is investigated by multiple antitrust authorities, including the US Federal Trade Commission, the European Commission, and the UK Competition and Markets Authority \parencite{livni_merced_2023}.

In parallel to the increased regulatory interest, the academic literature has also been steadily growing on the topic of hybrid platforms.
\textcite{hagiu2022should} investigates how practices, such as self-preferencing and steering can distort competition on hybrid platforms.
They argue that when such behavior is regulated, the platform selling its own products can be beneficial for consumers.
On the contrary, \textcite[]{anderson2021hybrid} find that even in the absence of such behavior, allowing hybrid operation might have negative consequences due to platforms having an incentive to exclude competitors from the market.
\textcite[postnote]{gutierrez2021welfare} shows that general conclusions are hard to draw, and welfare effects are platform and product specific.
...
This result is particularly striking in this otherwise distortion-free\footnote{
    In the benchmark model without bargaining, the platform's extent of hybrid operation does not affect consumer welfare.
} setting. 
The intuition is that hybrid operation increases the platform's bargaining power due to having its own products.
This in turn leads to a higher entry fee, leading to fewer entrants, which in turn leads to lower consumer surplus.
These observations highlight a so-far overlooked aspect of hybrid platforms that policymakers should be aware of.

The closest paper to this one is \textcite{anderson2021hybrid}, which utilizes a similar model and reaches similar conclusions.
I adapt many of the main features of thir model with a couple of crucial differences.
Instead of the platform setting a percentage entry fee and being able to commit to it, I assume that (1) platforms charge a lump-sum entry fee and that (2) this entry fee is the result of a negotiation process\footnote{
    The bargaining process is modeled using a concept from cooperative game theory, namely the Shapley-value.
...
Furthermore, in contrast to \textcite{hagiu2022should}, this model is free from any distortive behavior, such as self-preferencing or steering, and the negative results are purely driven by the platform's bargaining position.

This paper is also related to various other strands of the industrial organization literature.
In broad terms, it belongs to research agenda on understanding platforms and their role in the economy \parencite[e.g.][]{rochet2003platform,hagiu2004optimal,armstrong2006competition,evans2011platform,lee2014competing}.
...
Somewhat more generally, many results and concepts of this paper are also applicable outside the context of (multi-sided) platforms.
