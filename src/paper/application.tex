...
\Cref{fig:optimal_n_p_benchmark} illustrates this result.
...
\Cref{fig:optimal_n_p_bargaining} displays such an example.
...
Finally, \cref{prop:implied_entry_fee_comparative} can be viewed as a corollary of the previous theorem.
...
While I do not have a general result for the platform's profits in equilibrium, an important observation can be made.
No matter the form of the profit function, the platform's profits are always higher in the hybrid regime than if it exclusively sold its own products.
The intuition behind it is that the profit sharing rule is such that each player gets at least as much as they would make on their own\footnote{
    Which is only a positive amount for the platform, as the fringe cannot create any value on its own.
}.
The following proposition formalizes this idea.
\begin{proposition}
    \label{prop:hybrid_vs_retail_general}
    Under \cref{ass:additive_profit,ass:monotone_profits,ass:monotone_profits}, for any $N_C, N_P, N_F \geq 0$,
    \begin{align*}
        \pi_P(N_P, N_F, N_C) \geq \pi_P(N_P, 0, N_C).
    \end{align*}
\end{proposition}
\begin{proof}[Proof of \cref{prop:hybrid_vs_retail_general}]
    \begin{align*}
        \pi_P(N_P, N_F, N_C) &= N_C \int_0^1 w_P(s) f(N_P, s N_F) \ds \\
        &\geq N_C \int_0^1 w_P(s) f(N_P, 0) \ds \\
        &= \int_0^1 w_P(s) \ds N_C f(N_P, 0) \\
        &= \pi_P(N_P, 0, N_C),
    \end{align*}
    with the inequality being strict if $N_F > 0$ and $f$ is not constant its second argument on $[0, N_F]$.
\end{proof}
...
\begin{proof}[Proof of \cref{hybrid_vs_retail}]
    ...
    By \cref{prop:hybrid_vs_retail_general}, $\pi_P(N_P, N_F) \geq \pi_P(N_P, N_F)$ for any $N_F$, thus also for $N_F(N_P)$.
    When $N_F(N_P) > 0$, the inequality is strict.
\end{proof}
