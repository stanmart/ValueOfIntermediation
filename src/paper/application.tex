    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}[scale=0.8]
            \begin{axis}[xmin=0, xmax=1, ymin=0, ymax=1, samples at={0, 0.02, ..., 0.98, 1},
                ...
                \node[anchor=south east] at (axis cs: 0.6, 0.6) {$\tilde{f}(N_P, sN_F)$};
                \path[name path=bottom] (axis cs:0,0) -- (axis cs:1,0);
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Profit shares with $\tilde{f}$}
    \end{subfigure}
    \caption{Illustration of \cref{prop:share_of_platform}}
...
The intuition is that, even though the total size of the pie increases, the bargaining power of the fringe deteriorates so much that its share decreases not only in relative, but also in absolute terms.
