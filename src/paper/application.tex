...
Let us now describe each stage of the game in more detail.
I start with the final two subgames, as they are identical in both models.
Then I describe how entry fees and fringe entry is determined first in the benchmark model, and then highlight the differences in the bargaining model.
...
Due to this assumption, the timing of the model is also slightly different.
...
In practice, the resulting entry fees look as follows.
...
Let us denote these profits by $\pi^v_{F}(N_P,s N_F)$ and $\pi^v_{P}(N_P,s N_F)$, and the corresponding industry-wide total profits by $\Pi$ and $\Pi(N_P,s N_F, N_C)$.\footnote{
...
The timing of the two models are intended to be as similar as possible.
...
In the case of the benchmark model, if the platform set entry fees after fringe firms make their investment decisions, the platform would charge an entry fee that would make the fringe profits zero \emph{sans the investment cost}.
...
For a more thorough discussion of this assumption, see.
...
Finally, even taken at face value, this assumption about the platform's pricing is not as unrealistic as it might seem at first.
...
Therefore, if one is uncomfortable with the cooperative elements in the model, they can simply think of it as a more tractable representation of the one from \cref{sec:bargaining_microfoundation}.
...
Then, according to the definition of the Shappley value, every firm (including the platform) will get their average marginal contribution this industry-wide profit (with the average taken over all possible orderings of the firms).
...
The statements of these theorems are illustrated in \cref{fig:comparative_N_F,fig:welfare_aggregate,fig:platform_profits}.
