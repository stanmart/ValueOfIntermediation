...
\begin{corollary}
    ...
    \end{align*}
\end{corollary}
This result is a direct consequence of the facts that the aggregate is independent of the platform's product variety in the hybrid regime, and that consumer surplus only depends on the aggregate.
...
\begin{align*}
    K_F^i = \frac{\pi_F^v - \pi_F^t}{N_F} - I_F.
\end{align*}
As shown before, as the platform's product variety grows, its bargaining power also increases.
This can be thought of as an increase in (implied) entry fees $K_F^i$, which in turn discourages fringe entry.
\begin{proposition}
...
\Cref{sec:endogenizing_n_p} explores this phenomenon in more detail.
...
\begin{proposition}
    ...
    \begin{align*}
        K_F^i(N_P) < K_F^{opt} &\implies \frac{\dd \pi_P^t}{\dd N_P} > \frac{V_P}{V_F} I_F \\
        K_F^i(N_P) > K_F^{opt} &\implies \frac{\dd \pi_P^t}{\dd N_P} < \frac{V_P}{V_F} I_F.
    \end{align*}
\end{proposition}
...
\subsection{Discussion}
TODO: which assumptions are crucial for the results?
