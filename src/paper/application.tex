...
In the benchmark model, the platform will be able to set and commit to any lump-sum entry fee it wishes to.\footnote{
    The focus of this paper is on fees paid and entry decisions made by fringe producers, but I will also consider the case when consumers might also face entry fees or subsidies.
}
...
To model this in a theoretically sound, but still tractable way, I borrow a solution concept from cooperative game theory.
Similarly to a number of other papers in the industrial organization literature \parencite[e.g.][]{montez2007downstream,hart1990property,levy1997individual,inderst2003bargaining,brugemann2019intra}, I assume that the resulting allocation for each player is given by their Shapley value.
...
The following proposition establishes these both for the hybrid and the pure retail regimes.
...
\end{proposition}
Note, that even though the platform's profit is increasing in the number of its products, the optimal entry fee does not depend on it (\cref{fig:F_F_eq,fig:pi_P_eq}).
...
Therefore, the platform prefers to operate in hybrid mode, and does not want to exclude fringe firms from the market.\footnote{
...
\begin{proposition}
    ...
    \end{align*}
    The total profits of the whole fringe are given by
    \begin{align*}
...
that is, an increase in the platform's products leads to a more than proportional reduction in the number of fringe products (\cref{fig:fringe_entry_eq}).\footnote{
    The intuition behind this is demonstrated in \cref{fig:entry_fee}.
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[color=myblue,mark=none,thick] table[x=N_P,y=N_P]{\equilibriumProfitOneSidedLowLambda};
                \addplot[color=myred,mark=none,thick] table[x=N_P,y=N_F]{\equilibriumProfitOneSidedLowLambda};
                \addplot[color=black,mark=none,thick] table[x=N_P,y=A]{\equilibriumProfitOneSidedLowLambda};
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Platform product variety and number of entrants}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[color=myblue,mark=none,thick] table[x=N_P,y=pi_P]{\equilibriumProfitOneSidedLowLambda};
                \addplot[color=myred,mark=none,thick] table[x=N_P,y=pi_F]{\equilibriumProfitOneSidedLowLambda};
                \addplot[color=black,mark=none,thick] table[x=N_P,y=CS]{\equilibriumProfitOneSidedLowLambda};
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Profits and consumer surplus}
    \end{subfigure}
...
\begin{proof}[Proof of \cref{prop:aggregate_size_additive}]
    ...
    \begin{align*}
        \pi_F(N_P, N_F, N_C) = \beta N_C N_F \int_0^1 w_s s g'(\alpha N_P + s \beta N_F) \ds.
    \end{align*}
    ...
    \begin{align*}
        \frac{\partial N_F}{\partial N_P} = \frac{
            \alpha \beta N_C N_P \int_0^1 w(s) g''(\alpha N_P + s \beta N_F) \ds
        }{
            \underbracket{I'_F(N_F) - \beta N_C \int_0^1 w(s) g'(\alpha N_P + s \beta N_F) \ds}_{(1)} - \underbracket{\beta^2 N_C N_F \int_0^1 w(s) s g''(\alpha N_P + s \beta N_F) \ds}_{(2)}.
        }
    \end{align*}
    ...
    Next, observe that we can bound (2) from above using the fact that $s \in [0, 1]$:
    \begin{align*}
        (2) \leq \beta^2 N_C N_F \int_0^1 w(s) g''(\alpha N_P + s \beta N_F) \ds.
    \end{align*}
    For the same reason as before, it also bounds the whole expression from above.

    Putting it all together, we have that
    \begin{align*}
        \frac{\partial N_F}{\partial N_P} \leq \frac{\alpha \beta N_C N_P \int_0^1 w(s) g''(\alpha N_P + s \beta N_F) \ds}{\beta^2 N_C N_F \int_0^1 w(s) g''(\alpha N_P + s \beta N_F) \ds} = -\frac{\alpha}{\beta},
    \end{align*}
