...
However, it is equivalent to saying that the players negotiate over entry fees, as entry fees combined with equilibrium behavior later on determine the profit shares.
To keep the model as similar to the benchmark case as possible, I use the latter interpretation.

\begin{figure}
    \centering
    \begin{tikzpicture}[node distance=2cm, auto]
        \node[align=center] (entry_decision) {Potential entrants decide to invest \\ at cost $I_F$ \\ $[N_F]$};
        \node[align=center, draw, dashed] (entry_fee) [below of=entry_decision] {Entry fees are negotiated \\ between platform \\ and $N_F$ fringe firms \\ $[F_F]$};
        \node[align=center] (sales) [below of=entry_fee] {Platform and fringe \\ sell products \\ $[\pi_{Fi}^v, \pi_P^v]$};
        \node[align=center] (final) [below of=sales] {Final profits are realized \\ $[\pi_{Fi}^t, \pi_P^t]$};
        \draw[->] (entry_decision) -- (entry_fee);
        \draw[->] (entry_fee) -- (sales);
        ...
        \node[align=center] (choose_proposer) [below=1cm of foreach] {$F_i$ or $P$ is choosen randomly};
        \node[align=center] (proposal) [below of=choose_proposer] {Proposer proposes entry fee $F_{F_i}$};
        \node[align=center] (accept) [below left=2cm and -2cm of proposal] {Responder accepts $F_{F_i}$};
        \node[align=center] (reject) [below right=2cm and -2cm of proposal] {Responder rejects $F_{F_i}$};
        \node[align=center] (next_player) [below of=accept] {Move on to next \\ fringe firm $F_{F_{i+1}}$};
        ...
        \draw[->] (proposal) -- (accept.north);
        \draw[->] (proposal) -- (reject.north);
    ...
    \end{tikzpicture}
    \caption{Timing of the model with extensive-form bargaining. The panel on the right hand side details the bargaining procedure for determining the entry fees.}
