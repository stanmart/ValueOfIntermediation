...
class MarketStructure(Scene):
    def construct(self):
        ...
        self.next_section("investment")

        self.play(firms[-1].animate.set_opacity(0.2))
        self.play(Write(firm_eqs[0][0]), Write(firm_eqs[1][0]), Write(firm_eqs[2][0]))

        self.next_section("platform_entry")

        self.play(Create(firm_connections[0]), Create(firm_connections[1]))
        self.play(Write(firm_eqs[0][1]), Write(firm_eqs[1][1]))
        self.play(
        ...
        self.next_section("everyone_enters")
        self.play(Create(firm_connections[2]), Write(firm_eqs[2][1:]))

        self.next_section("shapley_values")
        self.play(
            Create(platform_brace_gr),
            Create(firm_shapleys[0]),
            Create(firm_shapleys[1]),
            Create(firm_shapleys[2]),
        )
